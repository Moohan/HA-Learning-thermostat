...
from custom_components.learning_thermostat import LearningThermostatData
...
async def test_setup_entry(hass: HomeAssistant, mock_config_entry: MockConfigEntry) -> None:
    ...
    # Check that runtime_data is populated
    entry = hass.config_entries.async_get_entry(mock_config_entry.entry_id)
    assert entry.runtime_data is not None
    assert isinstance(entry.runtime_data, LearningThermostatData)
