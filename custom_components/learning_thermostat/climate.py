...
class LearningThermostat(ClimateEntity, RestoreEntity):
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        _LOGGER.info(
            ...
            self._name, temperature, self._override_end_time
        )
    ...
    async def _async_prediction_loop(self, now=None):
        ...
        if self._is_override_active:
            if dt_util.now() < self._override_end_time:
                ...
                _LOGGER.info("%s: Manual override has ended.", self._name)
                self._is_override_active = False
        ...
        if predicted_temp is not None:
            _LOGGER.info("%s: Predicted temperature: %s", self._name, predicted_temp)
            ...
            _LOGGER.warning("%s: Failed to get a prediction.", self._name)
