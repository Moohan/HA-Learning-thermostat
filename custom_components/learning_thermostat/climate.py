...
class LearningThermostat(ClimateEntity, RestoreEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        attrs = {
            ...
            "model_trained": self._runtime_data.ml_core.is_trained,
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        if self._attr_preset_mode == PRESET_LEARNING_CONTROLLING:
            ...
            await self._runtime_data.data_collector.async_collect_data_point(temperature)
    ...
    async def _async_prediction_loop(self, now=None):
        ...
        if self._attr_hvac_mode != HVACMode.AUTO or not self._runtime_data.ml_core.is_trained:
            return
        ...
        sensor_data = {}
        for entity_id in self._runtime_data.sensor_entities:
            state = self.hass.states.get(entity_id)
            feature_name = sanitize_entity_id_for_feature(entity_id)
            sensor_data[feature_name] = state.state if state else "unknown"

        predicted_temp = await self._runtime_data.ml_core.async_predict_temperature(sensor_data)
