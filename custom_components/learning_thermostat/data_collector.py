...
class DataCollector:
    ...
    def _async_handle_initial_learning_state_change(self, event):
        ...
        if (
            ...
            _LOGGER.info(
                "Learning: Target temp for %s changed to %s. Collecting data.",
                self._climate_entity_id,
                new_temp,
            )
            self.hass.async_create_background_task(
                self.async_collect_data_point(new_temp), "collect_data_point"
            )
    ...
    async def async_collect_data_point(self, target_temperature):
        ...
        if self._on_data_collected and self._data_points_collected % 10 == 0:
