...
async def async_setup_entry(hass: HomeAssistant, entry: LearningThermostatConfigEntry) -> bool:
    ...
    sensor_entities_set = set(config.get("include_entities", []))
    ...
    for area_id in area_ids:
        ...
        for entity in entity_registry.entities.values():
            if (
                ...
                sensor_entities_set.add(entity.entity_id)

    sensor_entities = list(sensor_entities_set)
...
async def async_update_listener(hass: HomeAssistant, entry: LearningThermostatConfigEntry) -> None:
